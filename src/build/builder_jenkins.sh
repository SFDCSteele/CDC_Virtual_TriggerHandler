...
echo "#####builder_jenkins.sh: branch is $GITBRANCH---version 000.127...($ANT_HOME)...(GIT COMMITTER: $CI_COMMITTER_NAME)..."
echo "==========================================================================="
whoami
#ls -al
#pwd
...
for env in "${envs[@]}"; do
    ...
    then
        ...
        export execute_deploy=TRUE
        #
    elif [[ ( "$env" = "CARMA2PKG" ) || ( "$env" = "CARMA-ENH-000690_2" ) ]]
    then
        export envName="CARMA2PKG"
        echo "deploying to $env"
        export deploy_parms=" -DnoFlow=true "
        export packageXML="Package_$env.xml"
        export release_branch=$release_branch
